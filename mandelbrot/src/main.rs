...
// iterations it took for `c` to leave the circle if radius 2 centered on the
...
// bitmap.
//
// `bounds` is a pair giving the width and heigth of the bitmap. `pixel` is a
...
fn pixel_to_point(bounds: (usize, usize), pixel: (usize, usize), upper_left:(f64, f64),
...
fn render(pixels: &mut [u8], bounds: (usize, usize), upper_left: (f64, f64),
          lower_right: (f64, f64)) {
...
fn write_bitmap(filename: &str, pixels: &[u8], bounds: (usize, usize))
    ...
    try!(encoder.encode(&pixels, bounds.0 as u32, bounds.1 as u32,
                        ColorType::Gray(8)));
    Ok(())
}
...
fn main() {
    ...
    if args.len() != 5 {
        ...
        writeln!(std::io::stderr(),
                 "Example {} mandel.png 1000x750 -1.2,0.35 -1,0.20", args[0]).unwrap();
        std::process::exit(1);
    }
